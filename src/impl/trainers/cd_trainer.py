...
# from utils.vis import BilinearInterpolation, visualize_feature_map, image_proprecess, visualize_feature_distribution
from utils.metrics import (Meter, Precision, Recall, Accuracy, F1Score, IoU)
import cv2
from PIL import Image
import math
# 添加SANSAW
def get_boundary(label, edge_size=2):
...
class CDTrainer(Trainer):
    ...
    def train_epoch(self, epoch):
        ...
        for i, (t1, t2, tar) in enumerate(pb):
            ...
            tar_b = torch.empty_like(tar, device='cuda')
            for i in range(8):
                sub_tensor = tar[i, :, :]
                tensor_on_cpu = sub_tensor.cpu().to(torch.uint8)
                # 现在可以将tensor_on_cpu转换为NumPy数组
                sub_numpy = tensor_on_cpu.numpy()
                tar_b_ndarray = get_boundary(sub_numpy)
                tensor_on_gpu = torch.tensor(tar_b_ndarray, device='cuda')
                tar_b[i, :, :] = tensor_on_gpu
            ...
            pred = self._process_model_out(out[0:3])

            tars = []
            tars.append(tar)
            tars.append(tar)
            tars.append(tar_b)
            loss = self.criterion(pred, tars)
            loss += 0.05 * out[3].item()
            # loss += 0.01 * out[3].item()
    ...
    def evaluate_epoch(self, epoch):
        ...
        # Construct metrics
        metrics = (Precision(mode='accum'), Recall(mode='accum'), F1Score(mode='accum'), Accuracy(mode='accum'), IoU(mode='accum'))
        ...
        with torch.no_grad():
            for i, (name, t1, t2, tar) in enumerate(pb):
                ...
                tars.append(tar_b)
                # # vis = out[4]
                # BI = BilinearInterpolation(4, 4)
                # # imgsA_path = "/data/CDdata/LEVIRCDCutMix30/test/A/test_81/test_81_6.png"
                # # imgsB_path = "/data/CDdata/LEVIRCDCutMix30/test/B/test_81/test_81_6.png"
                # # save_path = "/data/CDdata/LEVIRCDCutMix30/test/temporal.png"
                # # imgsA_paths = "/data/CDdata/LEVIRCDCutMix30/testall/A"
                # # imgsB_paths = "/data/CDdata/LEVIRCDCutMix30/testall/B"
                # # save_paths = "/data/CDdata/LEVIRCDCutMix30/vis"
                # # for i in os.listdir(imgsA_paths):
                # #     imgsA_path = os.path.join(imgsA_paths, i)
                # #     imgsB_path = os.path.join(imgsB_paths, i)
                # #     save_path = os.path.join(save_paths, i)
                # #     t1vis = image_proprecess(imgsA_path)
                # #     t2vis = image_proprecess(imgsB_path)
                # #     t1vis, t2vis = self._prepare_data_vis(t1vis, t2vis)
                # #     vis = self.model(t1vis, t2vis)[4]
                # #     visualize_feature_map(vis, save_path, "drone", BI)
                # imgsA_path = "/data/CDdata/ChangeDetectionDataset/Real/subset/test/A/00949.jpg"
                # imgsB_path = "/data/CDdata/ChangeDetectionDataset/Real/subset/test/B/00949.jpg"
                # save_path = "/data/CDLab-master/BGCAFR.png"
                # t1vis = image_proprecess(imgsA_path)
                # t2vis = image_proprecess(imgsB_path)
                # t1vis, t2vis = self._prepare_data_vis(t1vis, t2vis)
                # vis = self.model(t1vis, t2vis)[4]
    ...
    def _prepare_data_vis(self, t1, t2):
        return t1.to(self.device), t2.to(self.device)
