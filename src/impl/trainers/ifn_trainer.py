...
from utils.losses import MixedLoss, CombinedLoss, CombinedLoss_threebranch, CombinedLoss_threebranch_BASloss
...
class IFNTrainer(CDTrainer_BCE):
    def _init_trainer(self):
        ...
        if self.ctx.get('cmb_coeffs') is not None:
            self.criterion = CombinedLoss_threebranch(self.criterion, self.ctx['cmb_coeffs'])
