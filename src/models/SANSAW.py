...
class SAN(nn.Module):
    ...
    def forward(self, x, masks):
        ...
        for i in self.selected_classes:
            ...
            out = mid*atten  # 8,128,32,32
            heatmap = torch.mean(out, dim=1, keepdim=True)  # 8,1,32,32

            class_region = self.set_class_mask_matrix(heatmap)  # 8,1,32,32  111
            out = self.Regional_Normalization(class_region,out)  # 8,128,32,32
            outs.append(out)
        out_ = sum(outs)
        out_ = self.relu(out_)
...
class SAW(nn.Module):
    def __init__(self, dim, relax_denom=0, classifier=None, work=True):
