...
__all__ = ['MyAttentionGlobal', 'MyAttention']
...
class MyAttention(nn.Module):
    ...
    def forward(self, hr_feat, lr_feat):
        ...
        hr_key = self.hr_key_conv(hr_feat)
        lr_query = self.lr_query_conv(lr_feat)
        # 使用函数f_similar计算lr_query和hr_key之间的相似性权重。
        weight = f_similar(lr_query, hr_key, self.kH, self.kW)
...
class MyAttentionNoGroup(nn.Module):
    def __init__(self, feat_dim, kW, kH):
        ...
        self.lr_query_conv = nn.Conv2d(feat_dim, feat_dim, kernel_size=3,
                                       padding=1)  # computation can be reduced by setting groups=feat_dim
        self.hr_key_conv = nn.Conv2d(feat_dim, feat_dim, kernel_size=3,
                                     padding=1)  # computation can be reduced by setting groups=feat_dim
...
class MyAttentionV2(nn.Module):
    def __init__(self, feat_dim, kW, kH):
        ...
        self.lr_query_conv = nn.Conv2d(feat_dim, feat_dim, kernel_size=3, padding=1,
                                       groups=8)  # computation can be reduced by setting groups=feat_dim
        self.hr_key_conv = nn.Conv2d(feat_dim, feat_dim, kernel_size=3, padding=1,
                                     groups=8)  # computation can be reduced by setting groups=feat_dim
    ...
    def forward(self, hr_feat, lr_feat):
        ...
        for i in range(scale_factor):
            for j in range(scale_factor):
                ...
                # weight = F.softmax(weight, dim=3)
                weight = self.softmax(weight)
                attention_result[..., i::scale_factor, j::scale_factor] = f_weighting(hr_value, weight, self.kH,
                                                                                      self.kW)
...
class MyAttentionLocalNew(nn.Module):
    ...
    def forward(self, hr_feat, lr_feat):
        ...
        # import pdb; pdb.set_trace()
        weight = f_similar(lr_query, downsample_hr_key, self.kH, self.kW).transpose(2, 3).transpose(1, 2).view(N,
                                                                                                               self.kH * self.kW,
                                                                                                               -1).transpose(
            1, 2)
...
class MyAttentionGlobal(nn.Module):
    def __init__(self, feat_dim, kScale=16):
        ...
        self.hr_value_conv = nn.Conv2d(feat_dim, feat_dim, kernel_size=3, padding=1,
                                       groups=feat_dim)  # , groups=feat_dim)
        self.attention = nn.MultiheadAttention(embed_dim=feat_dim, num_heads=1)
        # self.f_similar = f_similar
        # self.f_weighting = f_weighting
        self.kScale = kScale
...
class MyAttentionGlobalNoGroup(nn.Module):
    def __init__(self, feat_dim):
        ...
        self.lr_query_conv = nn.Conv2d(feat_dim, feat_dim // 4, kernel_size=3,
                                       padding=1)  # computation can be reduced by setting groups=feat_dim
        self.hr_key_conv = nn.Conv2d(feat_dim, feat_dim // 4, kernel_size=3,
                                     padding=1)  # computation can be reduced by setting groups=feat_dim
...
class ConvFusion(nn.Module):
    def __init__(self, feat_dim):
        ...
        self.fusion_conv = nn.Conv2d(2 * feat_dim, feat_dim, kernel_size=3,
                                     padding=1)  # computation can be reduced by setting groups=feat_dim
