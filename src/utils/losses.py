...
class CombinedLoss_threebranch(nn.Module):
    def __init__(self, critn, coeffs=1.0):
        super().__init__()
        self.critn = critn
        self.coeffs = coeffs
        # self.critn1 = BondaryLoss()

    def forward(self, preds, tars):
        if isinstance(self.coeffs, float):
            coeffs = [self.coeffs]*len(preds)
        else:
            coeffs = self.coeffs
        if len(coeffs) != len(preds):
            raise ValueError
        loss = 0.0
        loss += coeffs[0] * self.critn(preds[0], tars[0])
        loss += coeffs[1] * self.critn(preds[1], tars[1])
        loss += coeffs[2] * self.critn(preds[2], tars[2])
        return loss

class CombinedLoss_threebranch_BASloss(nn.Module):
    def __init__(self, critn, coeffs=1.0):
        super().__init__()
        self.critn = critn
        self.coeffs = coeffs
    ...
    def forward(self, preds, tars):
        ...
        loss += coeffs[1] * self.critn(preds[1], tars[1])
        loss += coeffs[2] * self.critn(preds[2], tars[2])
        loss += coeffs[3] * self.critn(preds[3], tars[3])
        return loss
